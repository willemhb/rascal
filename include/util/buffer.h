#ifndef rl_util_buffer_h
#define rl_util_buffer_h

#include "util/tpl/decl/buffer.h"

/* commentary

   Buffer types for IO and string operations.

   A buffer is an encoded alist. */

/* C types */
typedef BUFFER(ascii_t)  ascii_buffer_t;
typedef BUFFER(latin1_t) latin1_buffer_t;
typedef BUFFER(utf8_t) u8_buffer_t;
typedef BUFFER(utf16_t) u16_buffer_t;
typedef BUFFER(utf32_t) u32_buffer_t;
...
BUFFER_PUSHN(ascii_buffer, ascii_t);
...
BUFFER_PUSHN(latin1_buffer, latin1_t);
...
BUFFER_PUSHN(u16_buffer, utf16_t);
...
BUFFER_PUSHN(u32_buffer, utf32_t);
